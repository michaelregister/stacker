...
import { fetchSilverSpotPrice } from './services/geminiService';
import { SilverItem, SpotPriceData, User } from './types';

const COLORS = ['#94a3b8', '#64748b', '#475569', '#334155', '#1e293b', '#0f172a'];

const App: React.FC = () => {
  const [user, setUser] = useState<User | null>(() => {
    const saved = localStorage.getItem('silver-user');
    return saved ? JSON.parse(saved) : null;
  });

  const [isDarkMode, setIsDarkMode] = useState<boolean>(() => {
    const saved = localStorage.getItem('silver-theme');
    return saved ? saved === 'dark' : true; // Default to true
  });

  const [stack, setStack] = useState<SilverItem[]>([]);
  const [spotPrice, setSpotPrice] = useState<SpotPriceData | null>(null);
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      localStorage.setItem('silver-theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('silver-theme', 'light');
    }
  }, [isDarkMode]);
  ...
  useEffect(() => {
    if (user) {
      const saved = localStorage.getItem(`silver-stack-${user.email}`);
      setStack(saved ? JSON.parse(saved) : []);
    } else {
      setStack([]);
    }
  }, [user]);
  ...
  useEffect(() => {
    localStorage.setItem('silver-user', JSON.stringify(user));
    if (user) {
      localStorage.setItem(`silver-stack-${user.email}`, JSON.stringify(stack));
    }
  }, [user, stack]);
  ...
  useEffect(() => {
    const loadPrice = async () => {
      try {
        const data = await fetchSilverSpotPrice();
        setSpotPrice(data);
      } catch (error) {
  ...
  const totalValue = useMemo(() => {
    ...
    return totalOunces * spotPrice.price;
  }, [totalOunces, spotPrice]);

  const chartData = useMemo(() => {
    const categories: Record<string, number> = {};
    stack.forEach(item => {
      categories[item.category] = (categories[item.category] || 0) + item.totalOz;
    });
    return Object.entries(categories).map(([name, value]) => ({ name, value }));
  }, [stack]);

  const handleAddItem = (item: SilverItem) => {
  ...
  const exportToJson = () => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify({
      ...
      stack: stack,
      currentSpotPrice: spotPrice?.price
    }, null, 2));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `silver_portfolio_${user?.email || 'guest'}.json`);
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        {!user && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-2xl p-6 mb-8 flex items-center gap-4 text-blue-800 dark:text-blue-300 transition-colors animate-in slide-in-from-top duration-500">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">Sign in with Google to save your silver portfolio across devices.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard
            ...
            subValue={`${(totalOunces * 0.0311).toLocaleString(undefined, { maximumFractionDigits: 2 })} kg`}
            icon={
          ...
          <StatCard
            ...
            value={isPriceLoading ? "..." : `$${spotPrice?.price.toFixed(2)}`}
            subValue={spotPrice ? `Market Data Sourced via AI` : "Fetching latest market data..."}
            icon={
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold tracking-widest border-b border-slate-100 dark:border-slate-800">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-right">Unit Oz</th>
                        <th className="px-6 py-3 text-right">Total Oz</th>
                        ...
                        <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                          <td className="px-6 py-4">
                            ...
                            <div className="text-[10px] flex items-center gap-1.5 mt-1">
                              ...
                              <span className="text-slate-400 dark:text-slate-500 font-medium">{(item.purity * 100).toFixed(1)}% Silver</span>
                            </div>
                          </td>
          ...
          <div className="space-y-8">
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-6 flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">By Weight</span>
              </h2>
              <div className="h-64">
                {stack.length > 0 ? (
            ...
            <div className="bg-slate-900 dark:bg-slate-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden transition-colors">
              ...
              {spotPrice?.sources && spotPrice.sources.length > 0 ? (
                <div className="space-y-3 relative z-10">
                  {spotPrice.sources.map((source, i) => (
                    <a 
                      key={i}
                      href={source.uri} 
                      target="_blank" 
